...
test("200 percent zoom and WCAG text spacing retain reflow",async({page},testInfo)=>{test.skip(testInfo.project.name!=="chromium","layout probe runs once");await page.setViewportSize({width:1280,height:900});await page.goto("/");await page.evaluate(()=>{document.body.style.zoom="2";const style=document.createElement("style");style.textContent="*{line-height:1.5!important;letter-spacing:.12em!important;word-spacing:.16em!important}p{margin-bottom:2em!important}";document.head.append(style);});const overflow=await page.evaluate(()=>{const limit=document.documentElement.clientWidth+1;return Array.from(document.querySelectorAll<HTMLElement>("body *")).map(el=>{const r=el.getBoundingClientRect();return {tag:el.tagName,cls:el.className,id:el.id,left:Math.round(r.left),right:Math.round(r.right),width:Math.round(r.width)}}).filter(x=>x.right>limit||x.left<-1).slice(0,20)});expect(overflow).toEqual([]);});
...
test("artifact uses local pointer proximity in hero, projects and 404",async({page},testInfo)=>{
  ...
  await expect.poll(()=>page.locator("html").getAttribute("data-signal-input"),{timeout:3000}).toBe("ready");
  ...
  await page.evaluate(()=>document.getElementById("kairos")?.scrollIntoView({behavior:"instant",block:"center"}));
  await expect.poll(()=>page.locator("html").getAttribute("data-signal-state"),{timeout:3000}).toBe("kairos");
  box=await artifact.boundingBox();expect(box).not.toBeNull();
  await page.mouse.move(box!.x+box!.width*.82,box!.y+box!.height*.22);
  await expect.poll(async()=>Number(await page.locator("html").getAttribute("data-signal-proximity")),{timeout:2000}).toBeGreaterThan(.35);
  await page.goto("/missing-kinetic-page");
  const missing=page.locator(".not-found .signal-layer");
  await expect.poll(()=>missing.getAttribute("data-mode"),{timeout:5000}).toBe("webgl");
  await expect(missing).toHaveAttribute("data-variant","404");
  await expect.poll(()=>page.locator("html").getAttribute("data-signal-state"),{timeout:2000}).toBe("404");
  box=await missing.boundingBox();expect(box).not.toBeNull();
