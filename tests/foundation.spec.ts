...
test("Signal Form stops rendering after state settles",async({page},testInfo)=>{test.skip(testInfo.project.name!=="chromium","runtime diagnostics run once");await page.goto("/");await expect.poll(()=>page.locator("html").getAttribute("data-signal-frames"),{timeout:5000}).not.toBeNull();await page.getByRole("link",{name:/03 SAMBUT/}).click();await page.waitForTimeout(1800);const before=Number(await page.locator("html").getAttribute("data-signal-frames"));await page.waitForTimeout(500);const after=Number(await page.locator("html").getAttribute("data-signal-frames"));expect(after-before).toBeLessThanOrEqual(1);expect(Number(await page.locator("html").getAttribute("data-signal-calls"))).toBeLessThanOrEqual(12);});
