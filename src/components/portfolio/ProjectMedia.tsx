...
export function ProjectMedia({project,priority=false}:{project:Project;priority?:boolean}){
  ...
  function resetDepth(event:PointerEvent<HTMLElement>){
    const element=event.currentTarget;
    for(const [property,value] of [["--media-rotate-x","0deg"],["--media-rotate-y","0deg"],["--media-shift-x","0px"],["--media-shift-y","0px"],["--media-light-x","0px"],["--media-light-y","0px"]])element.style.setProperty(property,value);
  }
